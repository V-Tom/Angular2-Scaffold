 ...
 * 通过 id 获取 歌曲  mv
 */
const getMvByIdApi = 'music.163.com/mv'
const getMvById = (id) => {
  return spider.get(
    `${getMvByIdApi}?id=${id}`,
    null,
    ...
    if (embed.length != 0) {
      embed.attr('flashvars').split('&').forEach(x => {
        let i = x.indexOf('=')
        result[x.substring(0, i)] = x.substring(i + 1)
      })
    }
    return result
  })
}
exports.getMvById = getMvById

/**
 * 通过 id 下载  mv stream
 */
exports.requestMvStreamById = (id) => {
  return getMvById(id).then(result => {
    return request(result.murl).pipe(fs.createWriteStream(path.resolve(__dirname, `../public/mv/${id}.mp4`)))
  })
}
 ...
 * 获取本地mv 地址
 */
exports.getMVLocalPath = () => path.resolve(__dirname, '../public/mv')